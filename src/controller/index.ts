...
export const wrapGrid = (
  ...
  if (
    original_grid.length > new_height ||
    original_grid[0]?.length > new_width
  ) {
...
export const transform = (
  ...
  if (
    b.reduce(
      (acc, cur) => acc + cur.reduce((acc1, cur1) => acc1 + cur1, 0),
      0
    ) ===
    board.reduce(
      (acc, cur) => acc + cur.reduce((acc1, cur1) => acc1 + cur1, 0),
...
const hasSameDimensions = (boardA: number[][], boardB: number[][]): boolean => {
  return (
    boardA.length === boardB.length && boardA[0]?.length === boardB[0]?.length
  )
}
...
export const removeMatches = (board: number[][]): BoardState => {
  ...
  const grounded = splitted
    ...
    .reduce(
      (cur, acc) => join(cur, getCurrentGrid(acc) || emptyPiece()),
      emptyPiece()
    )
